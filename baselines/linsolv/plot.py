...
for curr_path in paths:
    ...
    if not results:
        ...
        continue
    print('loading {} ({})'.format(curr_path, len(results['total/epochs'])))
    with open(os.path.join(curr_path, '0.monitor.csv')) as f:
        first_line = f.readline()
    env_id=first_line[first_line.find("\"env_id\": \"")+11:-3]
    print(env_id)
    success_rate = np.array(results['eval/return'])
    epoch = np.array(results['total/epochs']) + 1
